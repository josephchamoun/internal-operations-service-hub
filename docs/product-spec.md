...
16. If a request remains untouched by the owning team for a defined period after submission, the system must automatically send a reminder or escalation notification to the owning team, unless a team member has silenced escalation for this specific request.
...
Given a message is sent by one side, when it is saved, then the other side (requester if team sent it, team if requester sent it) receives a notification, independent of whether they are actively viewing the request. If that side was already emailed about this request less than two minutes ago, the message is still saved and the email is not sent again.

**A burst of creates or messages is refused**
Given a signed-in person has created 5 requests in the last minute, when they try to create another, then the hub refuses it and stores nothing. Posting a message has the same cap of 5 per minute, counted separately from creates.
