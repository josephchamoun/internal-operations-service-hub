...
| One meaningful E2E test                                                 | Done                        | §6, E2E test (`test/requests.e2e-spec.ts`) — boots the real app, drives it over real HTTP via `supertest`, covers `401` (no token), `403` (denied claim), and `200` (allowed claim).                                                                                                                                                                                                                                                                                                                                                                |
...
| `GET /requests/:id/full`                                                                        | requester, owning-team member, or admin — anyone else gets `403`; every successful view is logged, no exceptions | `requests.service.ts`        |
...
**E2E test** (`test/requests.e2e-spec.ts`) — boots the real NestJS application in-memory and drives it with real HTTP requests via `supertest`, against the same dedicated `test.db` as the integration test above (never `dev.db`): logs in as two different seeded identities via `dev-login`, creates a request, then demonstrates both required authorization outcomes — a denied claim attempt from an actor not on the owning team (`403`), and an allowed claim from an actor on the owning team (`200`) — plus confirms an unauthenticated request is rejected (`401`).
